...
public class PersonaControllerTests {
	...
	void testinitCreationForm() throws Exception {
		mockMvc.perform(get("/register")).andExpect(status().isOk()).andExpect(model().attributeExists("persona"))
	...
	void testProcessCreationErrorForm() throws Exception {
		mockMvc.perform(post("/register")
				...
				.param("firstName", "George").with(csrf()).param("lastName", "Franklin").param("username", "person3")
				.param("password", "personi")).andExpect(view().name("redirect:/"));
	}

	@WithMockUser(value = "persona")
	@Test
	void testInitEditPersona() throws Exception {
		mockMvc.perform(get("/personas/edit/{personaId}", 4)).andExpect(status().isOk())
	...
	void testProccesEditPersona() throws Exception {
		mockMvc.perform(post("/personas/edit/{personaId}", 4).with(csrf()).param("firstName", "George")
				.param("lastName", "Franklin").param("username", "person3").param("password", "personi"))
	...
	void testInitEditPersonaPerfil() throws Exception {
		mockMvc.perform(get("/personas/editperfil/{personaId}", 4)).andExpect(status().isOk())
				.andExpect(model().attribute("persona", hasProperty("id", is(4))))
				.andExpect(model().attribute("persona", hasProperty("firstName", is("George"))))
				.andExpect(model().attribute("persona", hasProperty("lastName", is("Franklin"))))
				.andExpect(view().name(FORM));
	...
	void testProccesEditPersonaPerfil() throws Exception {
		mockMvc.perform(post("/personas/editperfil/{personaId}", 4).with(csrf()).param("firstName", "George")
				.param("lastName", "Franklin").param("username", "person3").param("password", "personi"))
