...
public class AmigosServiceTests {
	...
	public void deberiaGuardarInvitacion() {
		...
		invitacion.setId(1);
		User user1= userService.findUser("person1");
		User user2= userService.findUser("person2");
	...
	public void NoDeberiaGuardarInvitacion() {
		...
		invitacion.setId(2);
		User user1= userService.findUser("person1");
		User user2= userService.findUser("person1");
		invitacion.setUser1(user1);
		invitacion.setUser2(user2);
	...
	public void deberiaGuardarComoAmigos() {
		try {
			this.invitacionAmistadService.guardarAmigos("person1", "person2");
		} catch (DataAccessException e) {
		}
		// Comprobamos que person1 ha adquirido como amigo a person2
		Assertions.assertTrue(userService.findUser("person1").getAmigos().contains(userService.findUser("person2")));
	...
	public void noDeberiaGuardarComoAmigos() {
		try {
			this.invitacionAmistadService.guardarAmigos("person1", "person1");
		} catch (DataAccessException e) {
		}
		// Comprobamos que person1 no se puede incluir a si mismo como amigo
		Assertions.assertTrue(!userService.findUser("person1").getAmigos().contains(userService.findUser("person1")));
